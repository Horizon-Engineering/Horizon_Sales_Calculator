...
public class WeekView extends View {
    ...
    private int mMinHourHeight = 10; //no minimum specified (will be dynamic, based on screen)
    ...
    private Calendar getYesterdayDateString() {
        // DateFormat dateFormat = new SimpleDatecFormat("yyyy/MM/dd HH:mm:ss");
            ...
            switch (mCurrentScrollDirection) {
                case NONE: {
                    ...
                    if (Math.abs(distanceX) > Math.abs(distanceY)) {
                        if (distanceX > 0 && rightScroll ) {
                           // mCurrentScrollDirection = Direction.LEFT;
                        } else if(leftScroll && distanceX < 0){
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        for (int dayNumber=leftDaysWithGaps+1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            if (day.compareTo(calendar1) != 1 ){

                rightScroll = true;
                leftScroll = false;
                String dayLabel = getDateTimeInterpreter().interpretDate(day);
                setHeaderText(dayLabel);
                if (dayLabel == null)
               ...
               // rightScroll = false;
                ...
                if((day.compareTo(getYesterdayDateString()) != -1)){
                    leftScroll = true;
                }
                leftScroll = true;
                rightScroll = false;
